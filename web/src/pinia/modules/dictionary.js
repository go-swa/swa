import { findSwaDict } from '@/api/swaDict'
...
export const useDictionaryStore = defineStore('dictionary', () => {
  ...
  const getDictionary = async(type) => {
    if (dictionaryMap.value[type] && dictionaryMap.value[type].length) {
      ...
      const res = await findSwaDict({ type })
      if (res.code === 0) {
        const dictionaryRes = {}
        const dict = []
        res.data.resysDictionary.SwaDictail && res.data.resysDictionary.SwaDictail.forEach(item => {
          dict.push({
            label: item.label,
            value: item.value
          })
