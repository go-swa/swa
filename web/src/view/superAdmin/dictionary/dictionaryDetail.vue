  ...
  <div>
    ...
    <div class="gva-table-box">
      <div class="gva-btn-list">
        <el-button icon="plus" type="primary" @click="openDialog">新增字典项</el-button>
      </div>
      <el-table
          ref="multipleTable"
          :data="tableData"
      ...
      >
        ...
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button icon="edit" link type="primary" @click="updateSwaDictailFunc(scope.row)">变更</el-button>
            <el-popover v-model="scope.row.visible" placement="top" width="160">
              <p>确定要删除吗？</p>
              <div style="text-align: right; margin-top: 8px;">
                <el-button link type="primary" @click="scope.row.visible = false">取消</el-button>
                <el-button type="primary" @click="deleteSwaDictailFunc(scope.row)">确定</el-button>
              </div>
              <template #reference>
...
onBeforeRouteUpdate((to) => {
  if (to.name === 'dictionaryDetail') {
    searchInfo.value.swaDictID = to.params.id
    getTableData()
  }
})
...
const tableData = ref([])
const searchInfo = ref({swaDictID: Number(route.params.id)})
const onReset = () => {
  searchInfo.value = {swaDictID: Number(route.params.id)}
}
...
const getTableData = async () => {
  const table = await getSwaDictailList({
    page: page.value,
...
const updateSwaDictailFunc = async (row) => {
  const res = await findSwaDictail({ID: row.ID})
  type.value = 'update'
  if (res.code === 0) {
    formData.value = res.data.reSwaDictail
...
const deleteSwaDictailFunc = async (row) => {
  row.visible = false
  const res = await deleteSwaDictail({ID: row.ID})
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async () => {
  formData.value.swaDictID = Number(route.params.id)
  dialogForm.value.validate(async valid => {
    if (!valid) return
    let res
    switch (type.value) {
      case 'create':
        res = await createSwaDictail(formData.value)
        break
      case 'update':
        res = await updateSwaDictail(formData.value)
        break
      default:
        res = await createSwaDictail(formData.value)
        break
